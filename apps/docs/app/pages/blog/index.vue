...
const tags = computed(() => {
  ...
  return ["all", ...uniqueTags.map((tag) => tag.toLowerCase().replace(/^\w/, (c) => c.toUpperCase()))];
});

const filteredPosts = computed(() => {
  if (active.value === "all") return posts.value;
  return posts.value?.filter((post) => post.tags?.some((tag) => tag.toLowerCase() === active.value.toLowerCase()));
});
</script>
  ...
  <UPage v-if="page && posts">
    ...
    <UContainer class="pt-8">
      ...
      <div v-if="status !== 'pending'" class="flex flex-col gap-4">
        <BlogPost v-for="(post, index) in filteredPosts" :key="index" :post :to="post.path" />
      </div>
